...
export interface RecommendationCandidateGeneratorRules {
  /** @deprecated Use baseSlotsByType. */
  baseSlots?: number;
...
export function flexUsedFor(
  ...
  for (const itemId of [...new Set(itemIds)]) {
    ...
    if (item) usedByType[item.slotType] += 1;
  }
  return (Object.keys(usedByType) as InventorySlotType[]).reduce(
    (sum, type) => sum + Math.max(0, usedByType[type] - Math.max(0, rules.baseSlotsByType[type])),
    0,
  );
}
