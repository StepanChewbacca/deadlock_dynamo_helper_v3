...
describe('recommendation candidate generator', () => {
  ...
  it('enforces category-aware base slots instead of a universal total', () => {
    const definitions = [
      ...Array.from({ length: 5 }, (_, index) => item(index + 1, { slotType: 'weapon' })),
      item(6, { slotType: 'spirit' }),
    ];
    ...
    const rules = {
      ...
      activeCapacityEvidence: 'RECONSTRUCTED' as const,
    };
    ...
    expect(candidates.find((entry) => entry.actionId === 'BUY_ITEM:6')?.feasible).toBe(true);
  ...
  it('never allows more flex slots than the exact ruleset maximum', () => {
    const definitions = Array.from({ length: 8 }, (_, index) => item(index + 1));
    const input = decision(definitions, [1, 2, 3, 4, 5, 6, 7], 20_000);
    const rules = {
      ...DEFAULT_RECOMMENDATION_CANDIDATE_RULES,
      baseSlotsByType: { weapon: 4, vitality: 4, spirit: 4 },
      maxFlexSlots: 3,
    ...
    expect(candidates.find((entry) => entry.actionId === 'BUY_ITEM:8')?.reasons).toContain('SLOT_LIMIT_EXCEEDED');
  });
