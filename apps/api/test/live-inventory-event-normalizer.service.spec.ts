...
describe('LiveInventoryEventNormalizerService', () => {
  ...
  it('falls back to the unique player name when GEP reports inventory steam_id as zero', () => {
    const service = new LiveInventoryEventNormalizerService();
  ...
  it('does not carry a cached local Steam ID into a new match without one', () => {
    ...
    expect(normalized.events[2].payload).not.toMatchObject({ steam_id: oldSteamId });
  });
