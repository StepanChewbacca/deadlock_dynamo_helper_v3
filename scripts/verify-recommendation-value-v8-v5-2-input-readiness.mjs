...
const DATASET_SHA = 'e8b11e26df37ff1e17b334eda18ea2141cfb7fa78f0a34eaf95d448c22962235';
...
if (
  ...
  request?.operation !== 'RECOMMENDATION_VALUE_V8_V5_2_INPUT_READINESS' ||
  request?.readinessAuthorized !== true ||
  request?.valueV8TrainingAuthorized !== false ||
  request?.fullValueV8TrainingAuthorized !== false ||
  request?.productionRankingChanged !== false ||
  request?.passiveShadowAuthorized !== false ||
  request?.randomizedCanaryAuthorized !== false
) {
...
assertEqual(datasetManifest.artifact.sha256, DATASET_SHA, 'Dataset V6 manifest SHA-256');
...
const checks = {
  ...
  fullCorpus: manifest.build?.fullCorpus === true || audit.build?.fullCorpus === true,
  ...
  behaviorSupportCoverage: evaluation.releaseGate?.behaviorSupportCoverage >= 0.9,
  noMajorLowSupportGroups: Array.isArray(evaluation.releaseGate?.majorLowSupportGroups) && evaluation.releaseGate.majorLowSupportGroups.length === 0,
  extremeCandidateProbabilityRate: evaluation.releaseGate?.extremeCandidateProbabilityRate <= 0.01,
  floorSensitivity: evaluation.releaseGate?.probabilityFloorMaximumLogLossDelta <= 0.02,
  rawPropensityOutput: manifest.trainingContract?.propensityOutput === 'RAW_SOFTMAX_WITHIN_DECISION',
  noCandidateProbabilityFloor: manifest.trainingContract?.candidateProbabilityFloorApplied === false,
  noBehavioralIpsClipping: manifest.trainingContract?.ipsClippingApplied === false,
  observedOnlyFloorSensitivity: manifest.trainingContract?.probabilityFloorSensitivity === 'OBSERVED_PROPENSITY_CLIP_ONLY',
  futureNotTraining: manifest.trainingContract?.futureTestUsedForTraining === false,
  futureNotSelection: manifest.trainingContract?.futureTestUsedForSelection === false,
  sourceDatasetSha: manifest.source?.sha256 === DATASET_SHA || manifest.sourceDatasetSha256 === DATASET_SHA,
};
...
assertEqual(propensitySha256, propensityDescriptor.sha256, 'Behavioral propensity SHA-256');
...
const report = {
  ...
  propensityAudit,
  valueInputReady,
  valueLoaderV52IntegrationRequired: true,
...
if (!valueInputReady) {
  throw new Error(`Value V8 V5.2 input readiness failed: ${Object.entries(checks).filter(([, passed]) => passed !== true).map(([name]) => name).join(', ')}.`);
}
...
async function auditPropensities(path) {
  ...
  if (rowCount === 0) throw new Error('Behavioral V5.2 propensity artifact is empty.');
