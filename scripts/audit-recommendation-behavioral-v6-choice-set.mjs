...
const rowEvidencePath = process.env.BEHAVIORAL_V6_CHOICE_SET_ROWS_PATH?.trim();
...
for (const artifact of artifacts) {
  ...
  artifactByLineage.set(
    lineageKey(artifact.snapshot.policySha256, artifact.snapshot.catalogSha256),
    artifact,
  );
}
...
const snapshotLineageCount = new Map();
...
for await (const line of input) {
  ...
  const policyValue = artifact.policies.find(
    (value) => value.heroId === row.state.heroId,
  );
  ...
  const regeneratedKeys = result.mergedCandidates.map(
    (entry) => entry.candidate.actionKey,
  );
  const storedKeys = row.candidates.map((candidate) => candidate.actionKey);
  const mergedMatchesStoredDataset = sameArray(regeneratedKeys, storedKeys);
  if (!mergedMatchesStoredDataset) {
    regeneratedMergedMismatchCount += 1;
  }

  const observedKey = row.observedActionKey;
  const observedStored = row.candidates.find(
    (candidate) => candidate.actionKey === observedKey,
  );
  ...
  const time = timeBucket(row.state.gameTimeS);
  const economy = economyBand(row.state.netWorth);
  const phase = String(row.state.phase ?? 'UNKNOWN');
  const hero = String(row.state.heroId);
  const observedActionType = actionType(observedKey);
  observeMap(byTimeBucket, time, measurement);
  observeMap(byEconomyBand, economy, measurement);
  observeMap(byPhase, phase, measurement);
  observeMap(byHero, hero, measurement);
  observeMap(byActionType, observedActionType, measurement);
  ...
  if (rowEvidencePath) {
    rowEvidence.push({
      ...
      behavioralEligible: row.eligibility?.behavioralModel === true,
      mergedMatchesStoredDataset,
      ...measurement,
    });
  }

  if (selectedRowCount % 5_000 === 0) {
    console.log(
      JSON.stringify({
        progress: 'CHOICE_SET_AUDIT',
        selectedRowCount,
...
const report = {
  ...
  rowEvidence: {
    emitted: Boolean(rowEvidencePath),
    rowCount: rowEvidence.length,
  },
...
if (rowEvidencePath) {
  await writeFile(
    rowEvidencePath,
    `${rowEvidence.map((value) => JSON.stringify(value)).join('\n')}\n`,
    'utf8',
  );
}
...
function measureChoiceSet(input) {
  const primaryIndex = input.primaryCandidates.findIndex(
    (candidate) => candidate.actionKey === input.observedKey,
  );
  const supportIndex = input.supportCandidates.findIndex(
    (candidate) => candidate.actionKey === input.observedKey,
  );
  const mergedIndex = input.mergedCandidates.findIndex(
    (entry) => entry.candidate.actionKey === input.observedKey,
  );
  return {
...
function observe(acc, value) {
  ...
  if (value.primaryObservedRank !== undefined) {
    acc.primaryObservedRanks.push(value.primaryObservedRank);
  }
  if (value.supportObservedRank !== undefined) {
    acc.supportObservedRanks.push(value.supportObservedRank);
  }
  if (value.mergedObservedRank !== undefined) {
    acc.mergedObservedRanks.push(value.mergedObservedRank);
  }
  acc.historicalProbabilityGe001 +=
    value.historicalProbability >= 0.01 ? 1 : 0;
...
function finalize(acc) {
  return {
    ...
    supportUnionOnlyObservedRate: divide(
      acc.supportUnionOnlyObserved,
      acc.decisions,
    ),
    ...
    historicalProbabilitySupportAt001: divide(
      acc.historicalProbabilityGe001,
      acc.decisions,
    ),
    generatorScorePositiveRate: divide(
      acc.generatorScorePositive,
      acc.decisions,
    ),
  };
}
...
async function loadArtifacts(directory) {
  ...
  for (const entry of registry.snapshots) {
    const value = JSON.parse(
      await readFile(join(directory, entry.fileName), 'utf8'),
    );
    results.push(value);
  }
  return results;
...
function lineageKey(policySha, catalogSha) {
  return `${String(policySha ?? '').trim().toLowerCase()}:${String(
    catalogSha ?? '',
  )
    .trim()
    .toLowerCase()}`;
