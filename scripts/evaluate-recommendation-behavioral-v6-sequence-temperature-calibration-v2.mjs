...
const executorVersion = 'TUNING_MATCH_BALANCED_SPLIT_TEMPERATURE_CALIBRATION_2';
const partitionContract = 'MATCH_COUNT_DESC_FNV1A_TIE_GREEDY_2';
...
const matchDecisionCounts = await collectEligibleTuningMatchCounts();
const partition = createBalancedMatchPartition(matchDecisionCounts);
if (partition.calibrationDecisionCount < 2_000 || partition.validationDecisionCount < 2_000) {
  throw new Error(
    `Balanced MATCH calibration requires >=2000 decisions per partition; calibration=${partition.calibrationDecisionCount} validation=${partition.validationDecisionCount}.`,
  );
}

const calibrationAccumulators = new Map(
  temperatures.map((temperature) => [temperature, createAccumulator()]),
);
const validationAccumulators = new Map(
  temperatures.map((temperature) => [temperature, createAccumulator()]),
);
...
for await (const line of input) {
  ...
  const side = partition.assignment.get(String(selected.matchId));
  if (!side) throw new Error(`Missing calibration partition for match ${selected.matchId}.`);
  const prediction = predictRecommendationBehavioralV6Sequence(model, selected);
  const accumulators = side === 'CALIBRATION' ? calibrationAccumulators : validationAccumulators;
  if (side === 'CALIBRATION') calibrationDecisionCount += 1;
...
if (
  calibrationDecisionCount !== partition.calibrationDecisionCount ||
  validationDecisionCount !== partition.validationDecisionCount
) {
  throw new Error('Observed calibration partition counts differ from the frozen MATCH assignment.');
}
...
if (!selectedValidation || !uncalibratedValidation) {
  throw new Error('Selected or uncalibrated validation metrics are missing.');
}
const boosted = boostedSummary.tuning.model;
const v3LT = v3Summary.results.find((value) => value.variant === 'LT')?.tuningMetrics;
if (!v3LT) throw new Error('V3 LT TUNING metrics are missing.');

const selectedLate = supportForGroup(selectedValidation.metrics.groups, 'PHASE:LATE');
const selectedHighEconomy = supportForGroup(
  selectedValidation.metrics.groups,
  'ECONOMY:GE_20000',
);
const uncalibratedLate = supportForGroup(
  uncalibratedValidation.metrics.groups,
  'PHASE:LATE',
);
const uncalibratedHighEconomy = supportForGroup(
  uncalibratedValidation.metrics.groups,
  'ECONOMY:GE_20000',
);
...
const validationChecks = {
  ...
  highEconomySupportNotWorseThanUncalibrated:
    selectedHighEconomy >= uncalibratedHighEconomy,
};
...
const report = {
  ...
  source: {
    ...
    orderedHistorySignalObserved:
      sequenceSummary.continuationChecks.orderedHistorySignalObserved,
  },
  protocol: {
    tuningPartitionUnit: 'MATCH',
    partitionContract,
    partitionUsesObservedActionOrOutcome: false,
    partitionUsesOnlyEligibleDecisionCountsAndMatchId: true,
    calibrationMatchCount: partition.calibrationMatchCount,
    validationMatchCount: partition.validationMatchCount,
    calibrationDecisionCount,
    validationDecisionCount,
    decisionCountImbalance: Math.abs(calibrationDecisionCount - validationDecisionCount),
    temperatures,
    selectionRule: [
...
async function collectEligibleTuningMatchCounts() {
  ...
  if (counts.size < 4) throw new Error('Temperature calibration requires at least four TUNING matches.');
  return counts;
}

function createBalancedMatchPartition(matchDecisionCounts) {
  const entries = [...matchDecisionCounts.entries()].
    sort((left, right) =>
      right[1] - left[1] ||
      fnv1a32(left[0]) - fnv1a32(right[0]) ||
      left[0].localeCompare(right[0]),
    );
  const assignment = new Map();
  let calibrationDecisionCount = 0;
  let validationDecisionCount = 0;
  let calibrationMatchCount = 0;
  let validationMatchCount = 0;
  for (const [matchId, count] of entries) {
    const side =
      calibrationDecisionCount < validationDecisionCount
        ? 'CALIBRATION'
        : validationDecisionCount < calibrationDecisionCount
          ? 'VALIDATION'
          : fnv1a32(matchId) % 2 === 0
            ? 'CALIBRATION'
            : 'VALIDATION';
    ...
    if (side === 'CALIBRATION') {
      calibrationDecisionCount += count;
      calibrationMatchCount += 1;
    } else {
      validationDecisionCount += count;
      validationMatchCount += 1;
    }
  }
  return {
    assignment,
...
function validateInputs(modelArtifact, sequenceSummary, boostedSummary, v3Summary) {
  if (
    ...
    modelArtifact.model?.modelVersion !==
      RECOMMENDATION_BEHAVIORAL_V6_SEQUENCE_MODEL_VERSION ||
    modelArtifact.model?.probabilityContract !==
      RECOMMENDATION_BEHAVIORAL_V6_SEQUENCE_PROBABILITY_CONTRACT
  ) {
  ...
  if (
    boostedSummary.operation !==
      'RECOMMENDATION_BEHAVIORAL_V6_GROUPED_BOOSTED_LISTWISE_SCREEN' ||
...
function temperaturePrediction(prediction, observedActionKey, temperature) {
  const logits = prediction.candidates.map((candidate) => candidate.score / temperature);
...
function compareCalibrationCandidates(left, right) {
  return (
    Number(right.metrics.supportCoverage >= 0.84) -
      Number(left.metrics.supportCoverage >= 0.84) ||
    Number(right.metrics.floorSensitivityDelta <= 0.25) -
      Number(left.metrics.floorSensitivityDelta <= 0.25) ||
