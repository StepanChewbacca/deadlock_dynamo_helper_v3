...
const allCandidateCounts = [];
const eligibleCandidateCounts = [];
const observedRanks = [];
...
let observedActionMissingCount = 0;
...
for await (const line of input) {
  ...
  selectedRowCount += 1;
  allCandidateCounts.push(candidates.length);
  const observedIndex = candidates.findIndex((candidate) => candidate.actionKey === row.observedActionKey);
  if (observedIndex < 0) observedActionMissingCount += 1;
  if (Array.isArray(row.eligibility?.exclusionReasons)) {
    for (const reason of row.eligibility.exclusionReasons) increment(exclusionReasonCounts, reason);
  }
  if (row.eligibility?.behavioralModel !== true || observedIndex < 0) continue;

  eligibleRowCount += 1;
  eligibleCandidateCounts.push(candidates.length);
...
if (selectedRowCount === 0 || eligibleRowCount === 0) throw new Error('Pinned sample has no usable rows.');
...
const report = {
  ...
  sample: {
    selectedRowCount,
    eligibleRowCount,
    eligibilityRate: eligibleRowCount / selectedRowCount,
    observedActionMissingCount,
    observedActionCandidateCoverage: 1 - observedActionMissingCount / selectedRowCount,
    exclusionReasonCounts: sortedObject(exclusionReasonCounts),
    allCandidateCount: distribution(allCandidateCounts),
    eligibleCandidateCount: distribution(eligibleCandidateCounts),
    fractionCandidateCountGt100: fraction(eligibleCandidateCounts, (value) => value > 100),
    fractionCandidateCountGt128: fraction(eligibleCandidateCounts, (value) => value > 128),
    fractionCandidateCountGt200: fraction(eligibleCandidateCounts, (value) => value > 200),
