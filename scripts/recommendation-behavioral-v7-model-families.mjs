import { createRequire } from 'node:module';

const require = createRequire(import.meta.url);
const {
  createRecommendationBehavioralV7Model,
  predictRecommendationBehavioralV7,
  trainRecommendationBehavioralV7Decision,
  validateRecommendationBehavioralV7Model,
  RECOMMENDATION_BEHAVIORAL_V7_MODEL_VERSION,
  RECOMMENDATION_BEHAVIORAL_V7_FEATURE_VERSION,
  RECOMMENDATION_BEHAVIORAL_V7_PROBABILITY_CONTRACT,
  RECOMMENDATION_BEHAVIORAL_V7_OPTIMIZER,
} = require('/app/apps/api/dist/src/deadlock-live/recommendation-behavioral-v7.js');

export const BEHAVIORAL_V7_LINEAR_FAMILY = 'LINEAR_LISTWISE_V7_OBSERVABILITY_1';
export const BEHAVIORAL_V7_SEQUENCE_FAMILY = 'SEQUENCE_V7_OBSERVABILITY_V6_CAPACITY_1';
export const BEHAVIORAL_V7_MODEL_FAMILIES = [
  BEHAVIORAL_V7_LINEAR_FAMILY,
  BEHAVIORAL_V7_SEQUENCE_FAMILY,
];

export const BEHAVIORAL_V7_LINEAR_CONFIG = Object.freeze({
  hashDimension: 65_536,
});

export const BEHAVIORAL_V7_SEQUENCE_CONFIG = Object.freeze({
  historyLength: 16,
  hiddenDimension: 12,
  sequenceEmbeddingHashDimension: 4_096,
  contextEmbeddingHashDimension: 2_048,
  candidateEmbeddingHashDimension: 4_096,
  candidateBiasHashDimension: 8_192,
  recurrenceDecay: 0.8,
});

export const BEHAVIORAL_V7_LINEAR_TRAINING_OPTIONS = Object.freeze({
  learningRate: 0.05,
  l2: 0.0001,
  gradientClip: 1,
});

export const BEHAVIORAL_V7_SEQUENCE_TRAINING_OPTIONS = Object.freeze({
  learningRate: 0.03,
  l2: 0,
  gradientClip: 1,
});

const SEQUENCE_SCHEMA_VERSION = 1;
const SEQUENCE_MODEL_VERSION = 'RECOMMENDATION_BEHAVIORAL_V7_SEQUENCE_NEURAL_1_RAW_PROPENSITY';
const SEQUENCE_FEATURE_VERSION = 'RECOMMENDATION_BEHAVIORAL_V7_SEQUENCE_FEATURES_1_OBSERVABILITY_ORDERED_HISTORY';
const SEQUENCE_PROBABILITY_CONTRACT = 'RAW_SOFTMAX_WITHIN_DECISION';
const SEQUENCE_OBJECTIVE = 'GROUPED_LISTWISE_SOFTMAX_CROSS_ENTROPY_1';
const SEQUENCE_BASE_SCORE = 'NEGATIVE_LOG_CANDIDATE_RANK_1';
const SEQUENCE_ENCODER = 'RECURRENT_TANH_ORDERED_ACTION_ENCODER_1';
const SEQUENCE_OPTIMIZER = 'ONLINE_SGD_DECISION_GROUP_1';
const SEQUENCE_CAPACITY_REFERENCE = 'BEHAVIORAL_V6_SEQUENCE_NEURAL_SCREEN_1';

export function createBehavioralV7ModelFamily(familyId) {
  if (familyId === BEHAVIORAL_V7_LINEAR_FAMILY) {
    return {
      familyId,
      modelConfig: { ...BEHAVIORAL_V7_LINEAR_CONFIG },
      trainingOptions: { ...BEHAVIORAL_V7_LINEAR_TRAINING_OPTIONS },
      contracts: {
        modelVersion: RECOMMENDATION_BEHAVIORAL_V7_MODEL_VERSION,
        featureVersion: RECOMMENDATION_BEHAVIORAL_V7_FEATURE_VERSION,
        probabilityContract: RECOMMENDATION_BEHAVIORAL_V7_PROBABILITY_CONTRACT,
        optimizerContract: RECOMMENDATION_BEHAVIORAL_V7_OPTIMIZER,
        capacityContract: 'FIXED_HASH_65536_1',
        consumesV7Observability: true,
      },
      createModel: () => createRecommendationBehavioralV7Model(BEHAVIORAL_V7_LINEAR_CONFIG),
      predict: (model, row) => predictRecommendationBehavioralV7(model, row),
      train: (model, row) =>
        trainRecommendationBehavioralV7Decision(
          model,
          row,
          BEHAVIORAL_V7_LINEAR_TRAINING_OPTIONS,
        ).loss,
      validate: (model) => validateRecommendationBehavioralV7Model(model),
    };
  }

  if (familyId === BEHAVIORAL_V7_SEQUENCE_FAMILY) {
    return {
      familyId,
      modelConfig: { ...BEHAVIORAL_V7_SEQUENCE_CONFIG },
      trainingOptions: { ...BEHAVIORAL_V7_SEQUENCE_TRAINING_OPTIONS },
      contracts: {
        schemaVersion: SEQUENCE_SCHEMA_VERSION,
        modelVersion: SEQUENCE_MODEL_VERSION,
        featureVersion: SEQUENCE_FEATURE_VERSION,
        probabilityContract: SEQUENCE_PROBABILITY_CONTRACT,
        objective: SEQUENCE_OBJECTIVE,
        baseScoreContract: SEQUENCE_BASE_SCORE,
        encoderContract: SEQUENCE_ENCODER,
        optimizerContract: SEQUENCE_OPTIMIZER,
        capacityContract: SEQUENCE_CAPACITY_REFERENCE,
        consumesV7Observability: true,
      },
      createModel: createSequenceModel,
      predict: predictSequence,
      train: trainSequenceDecision,
      validate: validateSequenceModel,
    };
  }

  throw new Error(`Unsupported Behavioral V7 model family ${familyId}.`);
}

export function chooseBehavioralV7Champion(results) {
  if (!Array.isArray(results) || results.length !== BEHAVIORAL_V7_MODEL_FAMILIES.length) {
    throw new Error('Behavioral V7 bounded screen requires exactly the fixed Stage F model families.');
  }
  const byFamily = new Map(results.map((result) => [result.familyId, result]));
  for (const familyId of BEHAVIORAL_V7_MODEL_FAMILIES) {
    if (!byFamily.has(familyId)) {
      throw new Error(`Behavioral V7 bounded screen is missing ${familyId}.`);
    }
  }
  return [...results].sort(
    (left, right) =>
      right.tuning.supportCoverage - left.tuning.supportCoverage ||
      left.tuning.rawLogLoss - right.tuning.rawLogLoss ||
      BEHAVIORAL_V7_MODEL_FAMILIES.indexOf(left.familyId) -
        BEHAVIORAL_V7_MODEL_FAMILIES.indexOf(right.familyId),
  )[0];
}

function createSequenceModel() {
  const config = BEHAVIORAL_V7_SEQUENCE_CONFIG;
  const hidden = config.hiddenDimension;
  return {
    schemaVersion: SEQUENCE_SCHEMA_VERSION,
    modelVersion: SEQUENCE_MODEL_VERSION,
    featureVersion: SEQUENCE_FEATURE_VERSION,
    probabilityContract: SEQUENCE_PROBABILITY_CONTRACT,
    objective: SEQUENCE_OBJECTIVE,
    baseScoreContract: SEQUENCE_BASE_SCORE,
    encoderContract: SEQUENCE_ENCODER,
    optimizerContract: SEQUENCE_OPTIMIZER,
    capacityReference: SEQUENCE_CAPACITY_REFERENCE,
    ...config,
    sequenceEmbeddings: initializedArray(
      config.sequenceEmbeddingHashDimension * hidden,
      'SEQ',
    ),
    positionEmbeddings: initializedArray(config.historyLength * hidden, 'POS'),
    contextEmbeddings: initializedArray(
      config.contextEmbeddingHashDimension * hidden,
      'CTX',
    ),
    candidateEmbeddings: initializedArray(
      config.candidateEmbeddingHashDimension * hidden,
      'CAND',
    ),
    candidateBiases: initializedArray(config.candidateBiasHashDimension, 'BIAS'),
    trainedDecisionCount: 0,
  };
}

function validateSequenceModel(model) {
  const config = BEHAVIORAL_V7_SEQUENCE_CONFIG;
  if (
    model?.schemaVersion !== SEQUENCE_SCHEMA_VERSION ||
    model?.modelVersion !== SEQUENCE_MODEL_VERSION ||
    model?.featureVersion !== SEQUENCE_FEATURE_VERSION ||
    model?.probabilityContract !== SEQUENCE_PROBABILITY_CONTRACT ||
    model?.objective !== SEQUENCE_OBJECTIVE ||
    model?.baseScoreContract !== SEQUENCE_BASE_SCORE ||
    model?.encoderContract !== SEQUENCE_ENCODER ||
    model?.optimizerContract !== SEQUENCE_OPTIMIZER ||
    model?.capacityReference !== SEQUENCE_CAPACITY_REFERENCE
  ) {
    throw new Error('Unsupported Behavioral V7 sequence model contract.');
  }
  for (const [key, value] of Object.entries(config)) {
    if (model[key] !== value) {
      throw new Error(`Behavioral V7 sequence capacity drift detected for ${key}.`);
    }
  }
  const hidden = config.hiddenDimension;
  validateFiniteArray(
    model.sequenceEmbeddings,
    config.sequenceEmbeddingHashDimension * hidden,
    'sequenceEmbeddings',
  );
  validateFiniteArray(
    model.positionEmbeddings,
    config.historyLength * hidden,
    'positionEmbeddings',
  );
  validateFiniteArray(
    model.contextEmbeddings,
    config.contextEmbeddingHashDimension * hidden,
    'contextEmbeddings',
  );
  validateFiniteArray(
    model.candidateEmbeddings,
    config.candidateEmbeddingHashDimension * hidden,
    'candidateEmbeddings',
  );
  validateFiniteArray(
    model.candidateBiases,
    config.candidateBiasHashDimension,
    'candidateBiases',
  );
  if (!Number.isSafeInteger(model.trainedDecisionCount) || model.trainedDecisionCount < 0) {
    throw new Error('Behavioral V7 sequence trainedDecisionCount is invalid.');
  }
}

function predictSequence(model, row) {
  validateSequenceHeader(model);
  validateSequenceRow(row);
  const forward = forwardSequence(model, row);
  const scored = row.candidates.map((candidate) =>
    scoreSequenceCandidate(model, candidate, forward.finalHidden),
  );
  const probabilities = softmax(scored.map((value) => value.score));
  const candidates = scored.map((value, index) => ({
    ...value,
    probability: probabilities[index],
  }));
  let top = candidates[0];
  for (const candidate of candidates.slice(1)) {
    if (
      candidate.probability > top.probability ||
      (candidate.probability === top.probability &&
        candidate.actionKey.localeCompare(top.actionKey) < 0)
    ) {
      top = candidate;
    }
  }
  return {
    candidates,
    observedActionProbability:
      candidates.find((candidate) => candidate.actionKey === row.observedActionKey)
        ?.probability ?? 0,
    topActionKey: top.actionKey,
  };
}

function trainSequenceDecision(model, row) {
  validateSequenceHeader(model);
  validateSequenceRow(row);
  const options = BEHAVIORAL_V7_SEQUENCE_TRAINING_OPTIONS;
  const observedIndex = row.candidates.findIndex(
    (candidate) => candidate.actionKey === row.observedActionKey,
  );
  if (observedIndex < 0) {
    throw new Error('Behavioral V7 sequence observed action is outside candidate set.');
  }
  const forward = forwardSequence(model, row);
  const scored = row.candidates.map((candidate) =>
    scoreSequenceCandidate(model, candidate, forward.finalHidden),
  );
  const probabilities = softmax(scored.map((value) => value.score));
  const errors = probabilities.map((probability, index) =>
    (index === observedIndex ? 1 : 0) - probability,
  );
  const hiddenGradient = new Array(model.hiddenDimension).fill(0);

  for (let index = 0; index < row.candidates.length; index += 1) {
    const candidate = row.candidates[index];
    const error = errors[index];
    const embeddingOffset =
      candidateEmbeddingBucket(model, candidate) * model.hiddenDimension;
    for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
      const candidateValue = model.candidateEmbeddings[embeddingOffset + dimension];
      hiddenGradient[dimension] += error * candidateValue;
      const gradient = error * forward.finalHidden[dimension] - options.l2 * candidateValue;
      model.candidateEmbeddings[embeddingOffset + dimension] +=
        options.learningRate * clip(gradient, options.gradientClip);
    }
    const biasIndex = candidateBiasBucket(model, candidate);
    const bias = model.candidateBiases[biasIndex];
    model.candidateBiases[biasIndex] +=
      options.learningRate * clip(error - options.l2 * bias, options.gradientClip);
  }

  backpropagateSequence(model, forward, hiddenGradient, options);
  model.trainedDecisionCount += 1;
  return -Math.log(Math.max(probabilities[observedIndex], 1e-15));
}

function forwardSequence(model, row) {
  const contextTokens = sequenceContextTokens(row);
  const history = row.state.previousActionKeys;
  const sequenceTokens = history.slice(Math.max(0, history.length - model.historyLength));
  const hidden = new Array(model.hiddenDimension).fill(0);
  for (const token of contextTokens) {
    const offset = contextBucket(model, token) * model.hiddenDimension;
    for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
      hidden[dimension] += model.contextEmbeddings[offset + dimension] / contextTokens.length;
    }
  }
  for (let dimension = 0; dimension < hidden.length; dimension += 1) {
    hidden[dimension] = Math.tanh(hidden[dimension]);
  }
  const hiddenStates = [hidden.slice()];
  for (let index = 0; index < sequenceTokens.length; index += 1) {
    const token = sequenceTokens[index];
    const sequenceOffset = sequenceBucket(model, token) * model.hiddenDimension;
    const position = model.historyLength - sequenceTokens.length + index;
    const positionOffset = position * model.hiddenDimension;
    const previous = hiddenStates[hiddenStates.length - 1];
    const next = new Array(model.hiddenDimension);
    for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
      next[dimension] = Math.tanh(
        model.recurrenceDecay * previous[dimension] +
          model.sequenceEmbeddings[sequenceOffset + dimension] +
          model.positionEmbeddings[positionOffset + dimension],
      );
    }
    hiddenStates.push(next);
  }
  return {
    hiddenStates,
    contextTokens,
    sequenceTokens,
    finalHidden: hiddenStates[hiddenStates.length - 1],
  };
}

function sequenceContextTokens(row) {
  const tokens = [
    `HERO:${row.state.heroId}`,
    `TEAM:${row.state.team}`,
    `PHASE:${row.state.phase}`,
    `TIME:${Math.floor(Number(row.state.gameTimeS) / 300)}`,
    `ECONOMY:${economyBand(Number(row.state.netWorth))}`,
    `LEVEL:${Math.floor(finite(row.state.level) / 3)}`,
    `HEALTH:${healthBand(row)}`,
    `INVENTORY_SIZE:${inventorySizeBand(row)}`,
  ];
  for (const entry of row.state.inventoryItemCounts.slice(0, 12)) {
    tokens.push(`OWNED:${entry.itemId}:${Math.min(3, entry.count)}`);
  }
  for (const observation of row.observability) {
    const value = observation.missing
      ? 'MISSING'
      : observationValueBand(observation.value);
    tokens.push(`V7OBS:${observation.family}:${observation.fieldName}:${value}`);
    if (observation.alignmentAgeS !== undefined) {
      tokens.push(
        `V7OBS_AGE:${observation.fieldName}:${ageBand(observation.alignmentAgeS)}`,
      );
    }
  }
  return [...new Set(tokens)];
}

function scoreSequenceCandidate(model, candidate, hidden) {
  const embeddingOffset =
    candidateEmbeddingBucket(model, candidate) * model.hiddenDimension;
  let dot = 0;
  for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
    dot += hidden[dimension] * model.candidateEmbeddings[embeddingOffset + dimension];
  }
  const sequenceResidual = dot / Math.sqrt(model.hiddenDimension);
  const candidateBias = model.candidateBiases[candidateBiasBucket(model, candidate)];
  const baseScore = -Math.log(Math.max(1, Number(candidate.rank)));
  return {
    actionKey: candidate.actionKey,
    itemId: candidate.itemId,
    baseScore,
    sequenceResidual,
    candidateBias,
    score: baseScore + sequenceResidual + candidateBias,
  };
}

function backpropagateSequence(model, forward, finalHiddenGradient, options) {
  let hiddenGradient = finalHiddenGradient.map(
    (value) => value / Math.sqrt(model.hiddenDimension),
  );
  for (let index = forward.sequenceTokens.length - 1; index >= 0; index -= 1) {
    const currentHidden = forward.hiddenStates[index + 1];
    const token = forward.sequenceTokens[index];
    const sequenceOffset = sequenceBucket(model, token) * model.hiddenDimension;
    const position = model.historyLength - forward.sequenceTokens.length + index;
    const positionOffset = position * model.hiddenDimension;
    const previousGradient = new Array(model.hiddenDimension).fill(0);
    for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
      const localGradient =
        hiddenGradient[dimension] * (1 - currentHidden[dimension] ** 2);
      const sequenceValue = model.sequenceEmbeddings[sequenceOffset + dimension];
      const positionValue = model.positionEmbeddings[positionOffset + dimension];
      model.sequenceEmbeddings[sequenceOffset + dimension] +=
        options.learningRate *
        clip(localGradient - options.l2 * sequenceValue, options.gradientClip);
      model.positionEmbeddings[positionOffset + dimension] +=
        options.learningRate *
        clip(localGradient - options.l2 * positionValue, options.gradientClip);
      previousGradient[dimension] = model.recurrenceDecay * localGradient;
    }
    hiddenGradient = previousGradient;
  }

  const initialHidden = forward.hiddenStates[0];
  const contextScale = Math.max(1, forward.contextTokens.length);
  for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
    hiddenGradient[dimension] *= 1 - initialHidden[dimension] ** 2;
  }
  for (const token of forward.contextTokens) {
    const offset = contextBucket(model, token) * model.hiddenDimension;
    for (let dimension = 0; dimension < model.hiddenDimension; dimension += 1) {
      const value = model.contextEmbeddings[offset + dimension];
      const gradient = hiddenGradient[dimension] / contextScale - options.l2 * value;
      model.contextEmbeddings[offset + dimension] +=
        options.learningRate * clip(gradient, options.gradientClip);
    }
  }
}

function validateSequenceHeader(model) {
  if (
    model?.modelVersion !== SEQUENCE_MODEL_VERSION ||
    model?.featureVersion !== SEQUENCE_FEATURE_VERSION ||
    model?.probabilityContract !== SEQUENCE_PROBABILITY_CONTRACT ||
    model?.capacityReference !== SEQUENCE_CAPACITY_REFERENCE
  ) {
    throw new Error('Invalid Behavioral V7 sequence model header.');
  }
}

function validateSequenceRow(row) {
  if (
    row?.datasetVersion !== 'RECOMMENDATION_PRO_DECISION_DATASET_V7_OBSERVABILITY_1' ||
    row?.observabilityVersion !== 'RECOMMENDATION_OBSERVABILITY_V7_STRICT_PREDECISION_1' ||
    row?.choiceSet?.observedActionInjected !== false ||
    row?.choiceSet?.selectedAfterObservedAction !== false ||
    !Array.isArray(row?.observability) ||
    row.candidates.length < 2
  ) {
    throw new Error('Invalid Behavioral V7 sequence row.');
  }
}

function initializedArray(length, namespace) {
  return Array.from({ length }, (_, index) =>
    ((((fnv1a32(`${namespace}:${index}`) & 0xffff) / 0xffff) * 2 - 1) * 0.01),
  );
}

function validateFiniteArray(values, length, name) {
  if (!Array.isArray(values) || values.length !== length || values.some((value) => !Number.isFinite(value))) {
    throw new Error(`Behavioral V7 sequence ${name} is invalid.`);
  }
}

function sequenceBucket(model, token) {
  return fnv1a32(`SEQ:${token}`) % model.sequenceEmbeddingHashDimension;
}

function contextBucket(model, token) {
  return fnv1a32(`CTX:${token}`) % model.contextEmbeddingHashDimension;
}

function candidateEmbeddingBucket(model, candidate) {
  return (
    fnv1a32(`CAND:${candidate.actionKey}:${candidate.itemId}`) %
    model.candidateEmbeddingHashDimension
  );
}

function candidateBiasBucket(model, candidate) {
  return (
    fnv1a32(`BIAS:${candidate.actionKey}:${candidate.itemId}`) %
    model.candidateBiasHashDimension
  );
}

function fnv1a32(value) {
  let hash = 0x811c9dc5;
  for (let index = 0; index < value.length; index += 1) {
    hash ^= value.charCodeAt(index);
    hash = Math.imul(hash, 0x01000193);
  }
  return hash >>> 0;
}

function softmax(scores) {
  const maximum = Math.max(...scores);
  const weights = scores.map((score) => Math.exp(score - maximum));
  const total = weights.reduce((sum, value) => sum + value, 0);
  if (!Number.isFinite(total) || total <= 0) {
    throw new Error('Behavioral V7 sequence softmax normalization failed.');
  }
  return weights.map((value) => value / total);
}

function economyBand(value) {
  if (!Number.isFinite(value)) return 'UNKNOWN';
  if (value < 5_000) return 'LT_5000';
  if (value < 10_000) return '5000_9999';
  if (value < 15_000) return '10000_14999';
  if (value < 20_000) return '15000_19999';
  return 'GE_20000';
}

function healthBand(row) {
  const maximum = finite(row.state.maxHealth);
  const ratio = maximum > 0 ? finite(row.state.health) / maximum : 0;
  if (ratio < 0.25) return 'LT_025';
  if (ratio < 0.5) return '025_049';
  if (ratio < 0.75) return '050_074';
  return 'GE_075';
}

function inventorySizeBand(row) {
  const count = row.state.inventoryItemCounts.reduce(
    (sum, entry) => sum + entry.count,
    0,
  );
  if (count <= 3) return 'LE_3';
  if (count <= 6) return '4_6';
  if (count <= 9) return '7_9';
  return 'GE_10';
}

function observationValueBand(value) {
  if (typeof value === 'number') {
    if (!Number.isFinite(value)) return 'NONFINITE';
    const absolute = Math.abs(value);
    const width =
      absolute >= 10_000
        ? 5_000
        : absolute >= 1_000
          ? 500
          : absolute >= 100
            ? 50
            : absolute >= 10
              ? 5
              : 1;
    return String(Math.floor(value / width) * width);
  }
  if (Array.isArray(value)) return `ARRAY:${value.map((entry) => String(entry)).join('|')}`;
  return String(value);
}

function ageBand(ageS) {
  if (ageS <= 1) return 'LE_1';
  if (ageS <= 5) return 'LE_5';
  if (ageS <= 15) return 'LE_15';
  if (ageS <= 30) return 'LE_30';
  return 'GT_30';
}

function finite(value, fallback = 0) {
  return Number.isFinite(value) ? Number(value) : fallback;
}

function clip(value, maximumAbsoluteValue) {
  return Math.max(-maximumAbsoluteValue, Math.min(maximumAbsoluteValue, value));
}
