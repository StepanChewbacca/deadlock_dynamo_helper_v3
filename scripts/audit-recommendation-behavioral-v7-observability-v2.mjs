...
const entitySchemaRoot = required('BEHAVIORAL_V7_ENTITY_SCHEMA_ROOT');
...
const leakagePattern = /(winner|won|final.?outcome|match.?outcome|next.?action|future|three.?minute|five.?minute|ten.?minute)/i;
const families = [
  ...
  ['INVENTORY_LEGALITY', /(inventory|slot|flex|item|component|recipe|upgrade|stack|sold)/i],
  ...
  ['PURCHASE_TIMING_CONTEXT', /(purchase|buy|last.?purchase|inventory.?change|sold.?time)/i],
];
...
for (const entry of await readdir(timelineRoot, { withFileTypes: true })) {
  ...
  try {
    if ((await stat(path)).size > 0) timelineFiles.push({ matchId: entry.name, path });
  } catch {}
}
timelineFiles.sort((a, b) => a.matchId.localeCompare(b.matchId, undefined, { numeric: true }));
...
for (const file of timelineFiles) {
  ...
  for await (const line of input) {
    ...
    for (const [field, value] of Object.entries(payload)) {
      ...
      if (current.samples.length < 8 && !current.samples.includes(sample)) current.samples.push(sample);
...
const entityFields = [];
for (const entry of await readdir(entitySchemaRoot, { withFileTypes: true })) {
  if (!entry.isFile() || !entry.name.endsWith('.entity.ts')) continue;
  const text = await readFile(join(entitySchemaRoot, entry.name), 'utf8');
  const table = text.match(/@Entity\(['"]([^'"]+)['"]\)/)?.[1] ?? basename(entry.name, '.entity.ts');
  const propertyPattern = /@Column\([^)]*\)[\s\S]{0,220}?\n\s*([A-Za-z0-9_]+)!:\s*([^;\n]+);/g;
  for (const match of text.matchAll(propertyPattern)) {
    const property = match[1];
    const dataType = match[2].trim();
    const family = fieldFamily(property);
    if (!family) continue;
    const leakage = leakagePattern.test(property) ? 'REJECT_FUTURE_OR_OUTCOME' : 'UNKNOWN_ROW_TIMESTAMP';
    const existing = ['netWorth', 'kills', 'deaths', 'assists', 'heroId', 'team', 'purchaseTimeS'].includes(property);
    entityFields.push({
      canonicalFieldName: `database-schema.${table}.${property}`,
      family,
      ...
      alreadyRepresentedInDatasetV6: existing,
      leakageClassification: leakage,
      acceptableForStageB: !existing && leakage !== 'REJECT_FUTURE_OR_OUTCOME',
      rejectionReason: existing ? 'ALREADY_IN_V6' : leakage === 'REJECT_FUTURE_OR_OUTCOME' ? 'FUTURE_OR_OUTCOME_NAME' : undefined,
    });
  }
}

const inventory = [...timelineFields, ...entityFields].sort((a, b) => a.canonicalFieldName.localeCompare(b.canonicalFieldName));
...
const report = {
  ...
  source: {
    timelineRoot,
    entitySchemaRoot,
    eventFileCountDiscovered: timelineFiles.length,
